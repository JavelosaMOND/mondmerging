...
Route::middleware(['auth'])->group(function () {
    ...
    Route::prefix('admin')->name('admin.')->group(function () {
        ...
        Route::get('/user-archives', [AdminController::class, 'userArchives'])->name('user-archives');
        Route::get('/user-archives-json', [AdminController::class, 'userArchivesJson'])->name('user-archives.json');
        ...
        Route::post('/user-archives/{id}/unarchive', [AdminController::class, 'unarchiveUser'])->name('user-archives.unarchive');
        ...
        // Cluster Management
        Route::get('/clusters/{id}', [\App\Http\Controllers\ClusterController::class, 'showCluster'])->name('clusters.show');
    });
    ...
    // Facilitator Routes (new, using same controller for now)
    Route::prefix('facilitator')->name('facilitator.')->middleware(['auth', 'role:Facilitator'])->group(function () {
        Route::get('/dashboard', [ClusterController::class, 'dashboard'])->name('dashboard');
        // Barangays
        Route::get('/barangays', [ClusterController::class, 'barangays'])->name('barangays');
        Route::get('/barangays/{id}', [ClusterController::class, 'showBarangay'])->name('barangays.show');
    ...
    // Admin Facilitator User Management View
    Route::get('/admin/user-management-facilitator', [AdminController::class, 'userManagement'])->name('admin.user-management-facilitator');

    Route::post('/request-otp', [\App\Http\Controllers\OtpController::class, 'requestOtp'])->name('request.otp');
    Route::post('/verify-otp-change-password', [\App\Http\Controllers\OtpController::class, 'verifyOtpAndChangePassword'])->name('verify.otp.change.password');
});
